...
# citadel_alpha/cli.py — HLS Alpha Engine CLI.
# Google Python Style Guide.

"""Click CLI for HLS Alpha Engine (ISCF + MGD).
...
def hls_run(n_assets: int, n_periods: int, seed: int, output_dir: str) -> None:
    ...
    click.echo(click.style("  HLS Alpha Engine — ISCF + MGD (Synthetic)", fg="cyan", bold=True))
    click.echo(click.style("━" * 60, fg="cyan"))

    warmup = 120

    for signal_name, gen_fn, compute_fn, panel_attr in [
        ("ISCF", data_hls.generate_commodity_panel,
         lambda p, i: signals_hls.compute_iscf(
             p.spot[i], p.deferred[i], p.rvol[i], p.forward_returns[i],
             p.macro_beta[i],
             np.column_stack([p.trend_returns[i], p.momentum_returns[i], p.carry_returns[i]]),
         ), None),
        ("MGD", data_hls.generate_fx_panel,
         lambda p, i: signals_hls.compute_mgd(
             p.pmi_surprise[i], p.cpi_surprise[i], p.emp_surprise[i],
             p.fwd_expectation[i], p.roll_std[i], p.forward_returns[i],
             np.column_stack([p.trend_returns[i], p.momentum_returns[i], p.carry_returns[i]]),
         ), None),
    ]:
        ...
        sr = (float(np.mean(pnl_arr)) / max(float(np.std(pnl_arr, ddof=1)), 1e-8) * math.sqrt(252))
        ...
        ret_slice = np.array([float(np.mean(panel.forward_returns[i]))
                              for i in range(warmup, warmup + n_causal)])
        ...
        (out / f"{signal_name.lower()}_health.json").write_text(json.dumps(artifact, indent=2))
...
@click.option("--provider", default="yfinance", show_default=True,
              type=click.Choice(["yfinance", "hls"]))
@click.option("--output-dir", default="artifacts", show_default=True)
def hls_live(start: str, end: str, provider: str, output_dir: str) -> None:
    ...
    click.echo(click.style("━" * 60, fg="cyan"))
    click.echo(click.style(f"  HLS Live Run — Provider: {provider}", fg="cyan", bold=True))
    click.echo(click.style("━" * 60, fg="cyan"))
    ...
    try:
        ...
        for i in range(warmup, t):
            ...
            res = signals_hls.compute_iscf(
                iscf_data.spot[i], iscf_data.deferred[i], iscf_data.rvol[i],
                iscf_data.forward_returns[i], iscf_data.macro_beta[i], baseline,
            )
            ics.append(res.ic)
            pnl_list.append(float(np.mean(res.rank_score * iscf_data.forward_returns[i])))
        ...
        click.echo(click.style(f"\n{health.summary}",
                               fg="green" if not health.retirement_recommended else "red"))
        (out / "iscf_live_health.json").write_text(json.dumps({
            "signal": "ISCF", "gross_sr": sr, "mean_ic": health.mean_ic,
            "half_life_days": health.half_life.half_life_days,
            "retirement_recommended": health.retirement_recommended,
            "n_assets": n, "n_obs": t, "start": start, "end": end,
        }, indent=2))
    ...
    try:
        ...
        for i in range(warmup, t):
            ...
            res = signals_hls.compute_mgd(
                mgd_data.pmi_surprise[i], mgd_data.cpi_surprise[i],
                mgd_data.emp_surprise[i], mgd_data.fwd_expectation[i],
                mgd_data.roll_std[i], mgd_data.forward_returns[i], baseline,
            )
            ics.append(res.ic)
            pnl_list.append(float(np.mean(res.rank_score * mgd_data.forward_returns[i])))
        ...
        click.echo(click.style(f"\n{health.summary}",
                               fg="green" if not health.retirement_recommended else "red"))
        (out / "mgd_live_health.json").write_text(json.dumps({
            "signal": "MGD", "gross_sr": sr, "mean_ic": health.mean_ic,
            "half_life_days": health.half_life.half_life_days,
            "retirement_recommended": health.retirement_recommended,
            "n_assets": n, "n_obs": t, "start": start, "end": end,
        }, indent=2))
...
def hls_monitor(n_assets: int, n_periods: int, output_dir: str, sr_floor: float) -> None:
    ...
    click.echo(click.style("  HLS Signal Health Monitor + Rebalancing Engine", fg="cyan", bold=True))
    click.echo(click.style("━" * 70, fg="cyan"))
    ...
    for sig_name, panel in panels.items():
        ...
        for i in range(warmup, n_periods):
            if sig_name == "ISCF":
                bl = np.column_stack([panel.trend_returns[i], panel.momentum_returns[i], panel.carry_returns[i]])
                res = signals_hls.compute_iscf(
                    panel.spot[i], panel.deferred[i], panel.rvol[i],
                    panel.forward_returns[i], panel.macro_beta[i], bl)
            else:
                bl = np.column_stack([panel.trend_returns[i], panel.momentum_returns[i], panel.carry_returns[i]])
                res = signals_hls.compute_mgd(
                    panel.pmi_surprise[i], panel.cpi_surprise[i], panel.emp_surprise[i],
                    panel.fwd_expectation[i], panel.roll_std[i], panel.forward_returns[i], bl)
            ics.append(res.ic)
            pnl_list.append(float(np.mean(res.rank_score * panel.forward_returns[i])))
        ...
        sig_slice = np.array([
            float(np.mean((signals_hls.compute_iscf if sig_name=="ISCF" else signals_hls.compute_mgd)(
                *([panel.spot[i], panel.deferred[i], panel.rvol[i], panel.forward_returns[i], panel.macro_beta[i]]
                  if sig_name=="ISCF" else
                  [panel.pmi_surprise[i], panel.cpi_surprise[i], panel.emp_surprise[i],
                   panel.fwd_expectation[i], panel.roll_std[i], panel.forward_returns[i]]),
                np.column_stack([panel.trend_returns[i], panel.momentum_returns[i], panel.carry_returns[i]])
            ).rank_score))
            for i in range(warmup, warmup + n_c)
        ])
        ret_slice = np.array([float(np.mean(panel.forward_returns[i])) for i in range(warmup, warmup + n_c)])
        causal_result = causal.run_causal_stack(sig_name, sig_slice, ret_slice, n_bootstrap=50)
        ...
        click.echo(click.style(f"\n{causal_result.summary}", fg="green" if causal_result.recommendation=="PASS" else "yellow"))
        ...
        (out / f"{sig_name.lower()}_health.json").write_text(json.dumps(artifact, indent=2))

    click.echo(click.style(rebalance_summary(rebalance_decisions), fg="cyan"))

    click.echo()
    if any_issue:
        click.echo(click.style("\n⚠ One or more signals below KPI floor or recommend retirement.", fg="yellow", bold=True))
        sys.exit(1)
    else:
        click.echo(click.style("\n✓ All signals healthy. Continue 4x daily rebalancing.", fg="green", bold=True))
...
def benchmark(n_assets: int, n_reps: int) -> None:
    ...
    click.echo(click.style("  HLS Benchmark — ISCF Python hot-path", fg="cyan", bold=True))
    start = time.perf_counter()
    for _ in range(n_reps):
        signals_hls.compute_iscf(
            panel.spot[i], panel.deferred[i], panel.rvol[i],
            panel.forward_returns[i], panel.macro_beta[i], baseline,
        )
    elapsed = time.perf_counter() - start
    click.echo(f"  ISCF: {n_reps} reps in {elapsed:.3f}s "
